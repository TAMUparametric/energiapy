...
def check_operational_feasibility(design_model: ConcreteModel, all_scenario_dict: dict, scenarios2check: list,
                                  eps: float):
    counter = 0
    scen_op, model_op = build_design_model(eps=eps)
    ex_Cost = 0

    fix_design_variables(model2fix=model_op,
                         design_model=getattr(design_model.ef, [i[0] for i in design_model.scenarios()][0]))
    ...
    for scen_name in scenarios2check:
        ...
        model_op.constraint_nameplate_production_varying_capacity = make_constraint(instance=model_op,
                                                                                    ...
                                                                                    variable_x='P',
                                                                                    location_set=model_op.locations,
                                                                                    component_set=model_op.processes_varying_capacity,
                                                                                    loc_comp_dict=scen_op.location_process_dict,
                                                                                    b_factor=scen_op.capacity_factor,
        ...
        constraint_demand_penalty(instance=model_op, demand_scale_level=scen_op.demand_scale_level,
                                  ...
                                  demand_factor=scen_op.demand_factor,
                                  location_resource_dict=scen_op.location_resource_dict, sign='eq')

        constraint_demand_penalty_cost(instance=model_op, demand_scale_level=scen_op.demand_scale_level,
                                       demand_penalty_dict=scen_op.demand_penalty,
                                       demand_penalty_factor=scen_op.demand_penalty_factor)

        results_op = solve(scenario=scen_op, instance=model_op, solver='gurobi', name=scen_name,
                           solver_options=solver_options)

        print(
            f'######################## Finished solving {scen_name} for fill rate ({eps}) ({counter} of {len(scenarios2check)}) ########################')

        if results_op.output['termination'] == 'infeasible':
            print(f'Operational feasibility failed for {scen_name}')
            return 0, scen_name
        else:
            ex_Cost += all_scenario_dict[scen_name]['prob'] * pyoval(model_op.objective_cost_w_demand_penalty)
...
def round_robin_stochastic_design(all_scenario_dict: dict, eps: float = 0):
    ...
    iter_obj_dict = defaultdict(dict)
    ...
    while scen2add:
        if not selected_scenario_names:
            selected_scenario_names = random.choices(all_scenario_names,
                                                     weights=[all_scenario_dict[i]['prob'] for i in all_scenario_names],
                                                     k=1)
        ...
        iter_selected_scenario_names[eps][iter_count] = list(selected_scenario_dict.keys())
        ...
        ef_UI = ExtensiveForm(options, selected_scenario_names, design_scenario_creator,
        ...
        iter_obj_dict[eps][iter_count] = ef_UI.get_objective_value()
        iter_model_dict[eps][iter_count] = ef_UI

        if scen2add == 'start':
            # scenarios2check = [x for x in all_scenario_names if x not in selected_scenario_names]
            scenarios2check = random.sample(all_scenario_names, len(all_scenario_names))
        else:
        ...
        ex_Cost, scen2add = check_operational_feasibility(design_model=ef_UI, all_scenario_dict=all_scenario_dict,
                                                 scenarios2check=scenarios2check, eps=eps)

        if scen2add:
            selected_scenario_names.append(scen2add)

    return iter_selected_scenario_dict, iter_selected_scenario_names, iter_ssoln_dict, iter_fsc_dict, iter_model_dict, iter_obj_dict, ex_Cost
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    iter_selected_scenario_dict, iter_selected_scenario_names, iter_ssoln_dict, iter_fsc_dict, iter_model_dict, iter_obj_dict, ex_Cost = round_robin_stochastic_design(
        all_scenario_dict=load_scenario_dict, eps=fill_rate)
    end_time = time.time()
    ...
    for e in iter_fsc_dict:
        eps_fsc_dict[e] = iter_fsc_dict[e][list(iter_fsc_dict[e].keys())[-1]]

    print(f'Execution time: {end_time - start_time:.4f} seconds')
    print(f'Expected cost under uncertainty: {ex_Cost:.4f}')
    print(f'First Stage Cost: {eps_fsc_dict[list(eps_fsc_dict.keys())[0]]:.2f}')

    with open(f'RR_{fill_rate}_iter_fsc_dict.pkl', 'wb') as file:
        pickle.dump(iter_fsc_dict, file)

    with open(f'RR_{fill_rate}_iter_ssoln_dict.pkl', 'wb') as file:
        pickle.dump(iter_ssoln_dict, file)

    final_results_dict = {len(load_scenario_names): {'Expected Cost UI': ex_Cost,
                                                     'First Stage Cost': eps_fsc_dict[list(eps_fsc_dict.keys())[0]],
                                                     'Execution Time': end_time - start_time}}

    with open(f'RR_{fill_rate}_final_results_dict.pkl', 'wb') as file:
