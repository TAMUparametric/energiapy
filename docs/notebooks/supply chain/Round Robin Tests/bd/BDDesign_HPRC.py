...
if __name__ == '__main__':
    ...
    ls = LShapedMethod(options=bd_options, all_scenario_names=load_scenario_names,
                       scenario_creator=design_scenario_creator, scenario_creator_kwargs=scenario_creator_kwargs)
    ...
    with open(f'bdsoln_{len(load_scenario_names)}_{int(fill_rate * 10):02d}_UI.pkl', 'wb') as file:
        pickle.dump(variables, file)

    with open(f'time_{len(load_scenario_names)}_{int(fill_rate*10):02d}_UI.pkl', 'wb') as file:
