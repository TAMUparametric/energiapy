...
def round_robin_stochastic_design(all_scenario_dict: dict, eps: float = 0):
    ...
    while scen2add:
        ...
        scen2add = check_operational_feasibility(design_model=ef_UI, all_scenario_dict=all_scenario_dict,
                                                 scenarios2check=scenarios2check, eps=eps)

        if scen2add:
            selected_scenario_names.append(scen2add)

    return iter_selected_scenario_dict, iter_selected_scenario_names, iter_ssoln_dict, iter_fsc_dict, iter_model_dict
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    iter_selected_scenario_dict, iter_selected_scenario_names, iter_ssoln_dict, iter_fsc_dict, iter_model_dict = round_robin_stochastic_design(
        all_scenario_dict=load_scenario_dict, eps=1.0)
    end_time = time.time()
    ...
    with open(f'time_{fill_rate}.pkl', 'wb') as file:
        pickle.dump(end_time-start_time, file)