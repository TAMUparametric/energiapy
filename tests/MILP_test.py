...
def test_small_1L_mT_mO_MILP(m):
    ...
    assert m.release.sol(aslist=True) == pytest.approx([135.0, 157.5, 180.0, 67.5], rel=1e-9)
    ...
    assert m.invcapacity.sol(aslist=True) == pytest.approx([27.160493827160423], rel=1e-9)
    ...
    assert m.produce(m.power.lii, m.lii.charge.operate, m.q).sol(aslist=True) == pytest.approx(
        [6.882716049382594, 20.27777777777778, 0.0, 0.0], rel=1e-9
    )
    assert m.produce(m.power, m.lii.discharge.operate, m.q).sol(aslist=True) == pytest.approx(
    ...
    assert m.expend(m.power, m.lii.charge.operate, m.q).sol(aslist=True) == pytest.approx(
        [6.882716049382594, 20.27777777777778, 0.0, 0.0], rel=1e-9
    )
