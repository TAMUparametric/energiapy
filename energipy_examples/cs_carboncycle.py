...
Uranium = Resource(name='Uranium', cons_max= bigM, price=ur_price/(250/2), basis='kg', label='Uranium', block = 'energyfeedstock')
H2_C = Resource(name='H2_C', sell=True, store_max= bigM, loss=0.025/24, revenue=2, mile=1/(0.1180535*1.60934),
                        demand=True, basis='kg', label='Hydrogen - Local Cryo', block= 'resourcestorage') 
H2_L = Resource(name='H2_L', sell=True, store_max=bigM, demand=True, revenue=2,
...
CH4 = Resource(name='CH4', cons_max=bigM, price=1, basis='kg', label='Natural gas', block = 'materialfeedstock', varying = False)
CO2 = Resource(name='CO2', cons_max=bigM, basis='kg', label='Carbon dioxide', block = 'Resource')
CO2_DAC = Resource(
    name='CO2_DAC', basis='kg', label='Carbon dioxide - captured', block = 'carbonsequestration')
CO2_AQoff = Resource(
    name='CO2_AQoff', store_max=bigM, basis='kg', label='Carbon dioxide - sequestered', block = 'carbonsequestration')
CO2_EOR = Resource(
    name='CO2_EOR', store_max=bigM, basis='kg', label='Carbon dioxide - EOR', block = 'carbonsequestration')
CO2_Mat = Resource(
    name='CO2_Mat', store_max=bigM, basis='kg', label='Carbon dioxide - EOR', block = 'carbonsequestration')
...
demand = {(i,): 1000.0*(i+1) for i in range(scales.discretization_list[0])}
# demand = 1000
