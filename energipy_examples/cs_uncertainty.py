...
demand = {(i,j): 100.0 for i,j in product(range(scales.discretization_list[0]),range(scales.discretization_list[1]))}

#candidate cities (sinks)
#can only consume green hydrogen that is liquefied or dispensed through storage. 
Site1 = Location(name= '1', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site close to A')
Site2 = Location(name= '2', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site close to B')
Site3 = Location(name= '3', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site close to C')
Site4 = Location(name= '4', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site between A and B')
Site5 = Location(name= '5', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site between B and C')
Site6 = Location(name= '6', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site between C and A')
Site7 = Location(name= '7', processes = {H2_L_c, H2_L_d}, demand = demand, scales = scales, label= 'site between A, B and C')
...
graph.capacity_utilization(results = results_mplp, location = 'A')
#%%
