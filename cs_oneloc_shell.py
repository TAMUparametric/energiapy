...
HO = Location(name='HO', processes=ho_processes, demand=demand_dict, scales=scales, label='Houston')
...
case = Scenario(name='case', network=HO, scales=scales,  expenditure_scale_level=2,
                scheduling_scale_level=2, network_scale_level=0, demand_scale_level=1, label='shell milp case study')

# *-------------------------Generate reduced scenario------------------------------------
reduced_case = reduce_scenario(
    scenario=case, network=HO, periods=20, scale_level=1, method='agg_hierarchial')
...
results_red = solve(scenario=reduced_case, instance=milp_red,
                    solver='gurobi', name='red_onelocmilp', saveformat='.pkl')
...
graph.schedule(results=results_red, y_axis='P',
               component='PV', location='HO', usetex=True)
