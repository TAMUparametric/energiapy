...
H2_C = Resource(name='H2_C', sell=True, store_max= 10**10, loss=0.025/24, revenue=2, mile=1/(0.1180535*1.60934),
...
SMR = Process(name='SMR', intro_scale=smr_start, cost= {'CAPEX': 2400, 'Fixed O&M': 800, 'Variable O&M': 0.03, 'units': '$/kg', 'source': 'dummy'}, \
    conversion={Power: -1.11*10**(-3), CH4: -3.76, H2O: -23.7, H2_B: 1, CO2_Vent: 9.4979}, prod_max=bigM, gwp=0, trl='enterprise',
...
HO = Location(name='HO', processes={LiI_c, LiI_d, CAES_c, CAES_d, PSH_c, PSH_d, PV, WF, AKE, SMRH, H2_C_c,
                H2_C_d, H2_L_c, H2_L_d, DAC, EOR, AQoff_SMR, H2_Blue, H2_Green, SMR}, demand= ho_demand_dict, scales=scales, label='Houston')
...
# %%
graph.contribution(results=results_reduced_sl, y_axis='P_location', location='HO')
graph.contribution(results=results_sl, y_axis='P_location', location='HO')
graph.contribution(results=results_reduced_sl, y_axis='S_location', location='HO')
graph.contribution(results=results_sl, y_axis='S_location', location='HO')
graph.contribution(results=results_reduced_sl, y_axis='B_location', location='HO')
graph.contribution(results=results_sl, y_axis='B_location', location='HO')
graph.contribution(results=results_reduced_sl, y_axis='Cap_P', location='HO')
graph.contribution(results=results_sl, y_axis='Cap_P', location='HO')

graph.contribution(results=results_reduced_sl, y_axis='Cap_S', location='HO')
graph.contribution(results=results_sl, y_axis='Cap_S', location='HO')
...
# %%
graph.capacity_utilization(results=results_ml, location='HO')
...
# %%
results_ml.model_summary()

# %%
results_ml.output.keys()

# %%
results_ml.output['objective']
