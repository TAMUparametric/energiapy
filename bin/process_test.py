...
from src.energiapy.components.operation.process import (
    MaterialMode,
    ProcessMode,
    VaryingProcess,
)
...
def test_process_storage(process_storage):
    assert process_storage.conversion == {
        process_storage.storage: -1,
        process_storage.resource_storage: 1,
    }
    assert process_storage.processmode == ProcessMode.STORAGE
    assert process_storage.resource_req == {'Process1_Resource1_stored', 'Resource1'}
    assert process_storage.conversion_discharge == {
        process_storage.storage: 0.99,
        process_storage.resource_storage: -1,
    }
