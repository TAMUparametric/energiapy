...
def constraint_transport_imp_cost(instance: ConcreteModel, scheduling_scale_level: int = 0, trans_cost: dict = None,
                                  distance_dict: dict = None) -> Constraint:
    ...
    def transport_imp_cost_rule(instance, sink, source, resource, transport, *scale_list):
        if trans_cost is None:
            ...
            return instance.Trans_imp_cost[
                sink, source, resource, transport, scale_list[:scheduling_scale_level + 1]] == trans_cost[transport] * \
                distance_dict[(source, sink)] * instance.Trans_imp[
                    sink, source, resource, transport, scale_list[:scheduling_scale_level + 1]]

        instance.constraint_transport_imp_cost = Constraint(instance.sinks, instance.sources, instance.resources_trans,
                                                            instance.transports, *scales, rule=transport_imp_cost_rule,
...
def constraint_transport_cost(instance: ConcreteModel, scheduling_scale_level: int = 0) -> Constraint:
    ...
    def transport_cost_rule(instance, transport, *scale_list):
        return instance.Trans_cost[transport, scale_list[:scheduling_scale_level + 1]] == sum(
            instance.Trans_imp_cost[sink, source, resource, transport, scale_list[:scheduling_scale_level + 1]]
...
def constraint_transport_cost_network(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def transport_cost_network_rule(instance, transport, *scale_list):
        return instance.Trans_cost_network[transport, scale_list] == sum(
            instance.Trans_cost[transport, scale_] for scale_ in scale_iter)
...
def constraint_process_capex(instance: ConcreteModel, capex_dict: dict, network_scale_level: int = 0,
    ...
    def process_capex_rule(instance, location, process, *scale_list):
        if capex_dict[process] is not None:
            return instance.Capex_process[location, process, scale_list] == annualization_factor * capex_dict[process] * \
...
def constraint_process_incidental(instance: ConcreteModel, incidental_dict: dict, network_scale_level: int = 0,
                                  annualization_factor: float = 1,
                                  cost_dynamics: Costdynamics = Costdynamics.constant) -> Constraint:
    """Capital expenditure for each process at location in network
    ...
    instance.constraint_process_incidental = Constraint(
        instance.locations, instance.processes, *scales, rule=process_incidental_rule,
        doc='total incidental costs from processes')

    constraint_latex_render(process_incidental_rule)
    return instance.constraint_process_incidental


def constraint_process_fopex(instance: ConcreteModel, fopex_dict: dict, network_scale_level: int = 0,
                             annualization_factor: float = 1) -> Constraint:
    ...
    def process_fopex_rule(instance, location, process, *scale_list):
        if fopex_dict[process] is not None:
            return instance.Fopex_process[location, process, scale_list] == annualization_factor * fopex_dict[process] * \
                instance.Cap_P[location, process, scale_list]
        else:
...
def constraint_process_vopex(instance: ConcreteModel, vopex_dict: dict, network_scale_level: int = 0,
                             annualization_factor: float = 1) -> Constraint:
    """Fixed operational expenditure for each process at location in network
    ...
    def process_vopex_rule(instance, location, process, *scale_list):
        if vopex_dict[process] is not None:
            return instance.Vopex_process[location, process, scale_list] == annualization_factor * vopex_dict[process] * \
                instance.P_location[location, process, scale_list]
        else:
...
def constraint_location_capex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def location_capex_rule(instance, location, *scale_list):
        return instance.Capex_location[location, scale_list] == sum(
            instance.Capex_process[location, process_, scale_list] for process_ in instance.processes)
...
def constraint_location_incidental(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def location_incidental_rule(instance, location, *scale_list):
        return instance.Incidental_location[location, scale_list] == sum(
            instance.Incidental_process[location, process_, scale_list] for process_ in instance.processes)
...
def constraint_location_fopex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def location_fopex_rule(instance, location, *scale_list):
        return instance.Fopex_location[location, scale_list] == sum(
            instance.Fopex_process[location, process_, scale_list] for process_ in instance.processes)
...
def constraint_location_vopex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def location_vopex_rule(instance, location, *scale_list):
        return instance.Vopex_location[location, scale_list] == sum(
            instance.Vopex_process[location, process_, scale_list] for process_ in instance.processes)
...
def constraint_network_capex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def network_capex_rule(instance, *scale_list):
        return instance.Capex_network[scale_list] == sum(
            instance.Capex_location[location_, scale_list] for location_ in instance.locations)
...
def constraint_network_incidental(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def network_incidental_rule(instance, *scale_list):
        return instance.Incidental_network[scale_list] == sum(
            instance.Incidental_location[location_, scale_list] for location_ in instance.locations)
...
def constraint_network_vopex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def network_vopex_rule(instance, *scale_list):
        return instance.Vopex_network[scale_list] == sum(
            instance.Vopex_location[location_, scale_list] for location_ in instance.locations)
...
def constraint_network_fopex(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def network_fopex_rule(instance, *scale_list):
        return instance.Fopex_network[scale_list] == sum(
            instance.Fopex_location[location_, scale_list] for location_ in instance.locations)
...
def constraint_process_land_cost(instance: ConcreteModel, land_dict: dict, land_cost_dict: dict,
    ...
    def process_land_cost_rule(instance, location, process, *scale_list):
        return instance.Land_cost_process[location, process, scale_list] == land_cost_dict[location] * land_dict[
            process] * instance.Cap_P[location, process, scale_list]

    instance.constraint_process_land_cost = Constraint(
        instance.locations, instance.processes, *scales, rule=process_land_cost_rule,
        doc='land cost for process at location')
...
def constraint_location_land_cost(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def location_land_cost_rule(instance, location, *scale_list):
        return instance.Land_cost_location[location, scale_list] == sum(
            instance.Land_cost_process[location, process_, scale_list] for process_ in instance.processes)
...
def constraint_network_land_cost(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def network_land_cost_rule(instance, *scale_list):
        return instance.Land_cost_network[scale_list] == sum(
            instance.Land_cost_location[location_, scale_list] for location_ in instance.locations)
