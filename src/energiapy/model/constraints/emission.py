...
def constraint_carbon_emission_location(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def carbon_emission_location_rule(instance, location, *scale_list):
        return instance.carbon_emission_location[location, scale_list] == instance.S_location[
            location, 'CO2_Vent', scale_list]
...
def constraint_carbon_emission(instance: ConcreteModel, carbon_bound: float, network_scale_level: int = 0,
                               carbon_reduction_percentage: float = 0.0) -> Constraint:
    ...
    def carbon_emission_rule(instance, location, *scale_list):
        return instance.S_location[location, 'CO2_Vent', scale_list] <= (
                    100.0 - carbon_reduction_percentage) * carbon_bound / 100.0
...
def constraint_global_warming_potential_process(instance: ConcreteModel, process_gwp_dict: dict,
    ...
    def global_warming_potential_process_rule(instance, location, process, *scale_list):
        return instance.global_warming_potential_process[location, process, scale_list] == process_gwp_dict[location][
            process] * instance.Cap_P[location, process, scale_list]

    instance.constraint_global_warming_potential_process = Constraint(
        instance.locations, instance.processes, *scales, rule=global_warming_potential_process_rule,
        doc='global warming potential for the each process')
    constraint_latex_render(global_warming_potential_process_rule)
    return instance.constraint_global_warming_potential_process


def constraint_global_warming_potential_material_mode(instance: ConcreteModel, material_gwp_dict: dict,
                                                      process_material_mode_material_dict: dict,
                                                      network_scale_level: int = 0) -> Constraint:
    ...
    def global_warming_potential_material_mode_rule(instance, location, process, material_mode, *scale_list):
        if material_mode in process_material_mode_material_dict[process]:
            return instance.global_warming_potential_material_mode[location, process, material_mode, scale_list] == \
                sum(process_material_mode_material_dict[process][material_mode][material] * material_gwp_dict[location][
                    material]
                    for material in instance.materials if
                    material in process_material_mode_material_dict[process][material_mode]) * instance.Cap_P_M[
                    location, process, material_mode, scale_list]
        else:
            return Constraint.Skip

    instance.constraint_global_warming_potential_material_mode = Constraint(
        instance.locations, instance.processes_materials, instance.material_modes, *scales,
        rule=global_warming_potential_material_mode_rule, doc='global warming potential for the each material')
...
def constraint_global_warming_potential_material(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def global_warming_potential_material_rule(instance, location, process, *scale_list):
        return instance.global_warming_potential_material[location, process, scale_list] == sum(
            instance.global_warming_potential_material_mode[location, process, material_mode_, scale_list] for
            material_mode_ in instance.material_modes)

    instance.constraint_global_warming_potential_material = Constraint(
        instance.locations, instance.processes_materials, *scales, rule=global_warming_potential_material_rule,
        doc='global warming potential for the each material')
...
def constraint_global_warming_potential_resource_consumption(instance: ConcreteModel, resource_gwp_dict: dict,
                                                             network_scale_level: int = 0) -> Constraint:
    ...
    def global_warming_potential_resource_consumption_rule(instance, location, resource, *scale_list):
        return instance.global_warming_potential_resource_consumption[location, resource, scale_list] == \
            resource_gwp_dict[location][resource] * instance.C_location[location, resource, scale_list]

    instance.constraint_global_warming_potential_resource_consumption = Constraint(
        instance.locations, instance.resources_purch, *scales, rule=global_warming_potential_resource_consumption_rule,
        doc='global warming potential for the each resource consumed')
...
def constraint_global_warming_potential_resource_discharge(instance: ConcreteModel, resource_gwp_dict: dict,
                                                           network_scale_level: int = 0) -> Constraint:
    ...
    def global_warming_potential_resource_discharge_rule(instance, location, resource, *scale_list):
        return instance.global_warming_potential_resource_discharge[location, resource, scale_list] == \
            resource_gwp_dict[location][resource] * instance.S_location[location, resource, scale_list]

    instance.constraint_global_warming_potential_resource_discharge = Constraint(
        instance.locations, instance.resources_sell, *scales, rule=global_warming_potential_resource_discharge_rule,
        doc='global warming potential for the each resource discharged')
...
def constraint_global_warming_potential_resource(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    instance.constraint_global_warming_potential_resource = Constraint(
        instance.locations, instance.resources, *scales, rule=global_warming_potential_resource_rule,
        doc='global warming potential for the each resource')
...
def constraint_global_warming_potential_location(instance: ConcreteModel, network_scale_level: int = 0) -> Constraint:
    ...
    def global_warming_potential_location_rule(instance, location, *scale_list):
        gwp_process = sum(
            instance.global_warming_potential_process[location, process_, scale_list] for process_ in
            instance.processes)
        gwp_resource = sum(
            instance.global_warming_potential_resource[location, resource_, scale_list] for resource_ in
            instance.resources)
        gwp_material = sum(
            instance.global_warming_potential_material[location, material_, scale_list] for material_ in
            instance.processes_materials)

        return instance.global_warming_potential_location[
            location, scale_list] == gwp_process + gwp_resource + gwp_material

    instance.constraint_global_warming_potential_location = Constraint(
        instance.locations, *scales, rule=global_warming_potential_location_rule,
        doc='global warming potential for the each location')
...
def constraint_global_warming_potential_network_reduction(instance: ConcreteModel, network_scale_level: int = 0,
                                                          gwp_reduction_pct: float = 0, gwp: float = 0) -> Constraint:
    ...
    instance.constraint_global_warming_potential_network_reduction = Constraint(
        *scales, rule=global_warming_potential_network_reduction_rule,
        doc='global warming potential for the whole network')
...
def constraint_global_warming_potential_network_bound(instance: ConcreteModel, gwp_bound: float,
                                                      network_scale_level: int = 0) -> Constraint:
    ...
    return Constraint(
        *scales, rule=global_warming_potential_network_bound_rule,
        doc='global warming potential bound for the whole network')


def constraint_global_warming_potential_20_resource(instance: ConcreteModel, emission_dict: dict,
                                                    network_scale_level: int = 0) -> Constraint:
    """calculates global warming potential for each process
    ...
    def global_warming_potential_process_rule(instance, location, process, resource, *scale_list):
        return instance.global_warming_potential_20_resource[location, resource, scale_list] == \
            emission_dict[location][process][resource] * instance.Cap_P[location, process, scale_list]
