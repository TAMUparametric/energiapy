...
class Data:
    ...
    Data can also be defined externally and passed instead of a DataFrame. 
    ...
    def __post_init__(self):
        ...
        if isinstance(self.data, Data):
            ...
            raise ValueError(
                f'{str(self.aspect.name).lower()} factor for {self.component.name}: please provide DataFrame, Data, or a dict of either with the nominal value as key')
    ...
    def __lt__(self, other):
        if isinstance(other, (int, float)) and self.bound == Bound.UPPER:
            return False
        elif isinstance(other, Data) and other.bound == Bound.LOWER:
            return False
        else:
            return True
